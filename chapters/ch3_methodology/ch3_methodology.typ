...
The data that is currently in us was easely collected. The damage data was kindly provided by K. Liechti from the #abbr.a[WSL] after our request by mail@liechtiREAnfrageZur2024. To collcet the weather data the first approach was to use governant date provided by Meteo Swiss @MeteoSwissIDAWEBLogin. Due to the behaviour of the website and teh weather data being raw data from the measurement stations this idea was abolished. By researching the open-meteo API@zippenfenigOpenMeteocomWeatherAPI2023 was discovered. To avoid imense cost a free research key was requested which got kindly provided@zippenfenigProfessionalAPIKey. To receive data about teh ground the first source was also a governant (map.geo.admin)@MapsSwitzerlandSwiss. The format of the data was mostly not in a format that could be used with small expense. another possible resource was teh GIS Browser @GISBrowserGeoportalKanton which is the Cantonal equivalent of map.geo.admin but it burged the same issues as teh governant one. Considering that there are new buildings built in switzerland and the Confederation is searching for a nuclear repository@VergrabenUndVergessen2019, teh assumption was, some public body has to have those data. First the building constuction office of affoltern am albis was contacted@duchaudPhoneCallHochbau2025 which revered to the building constuction office of the canton. teh Cantons office@PhoneCallHochbau2025 also denied the possesion of such data and refered to the Office for Spatial Development. The correspondence partner from the Office for Spatial development@muellerPhoneCallAmt2025 coudln't provide any further information or person to contact. The provided suggestion was to search on GIS Broser@GISBrowserGeoportalKanton or map.geo.admin@MapsSwitzerlandSwiss. Due to the previous unsuccessful attemps, GIS Helpdesk was contacted@ueltschiBodenbeschaffenheitskarte and the suggestes aolution@fachstellegisAREJIRAGIS2262EXTERN was to use GIS Broser@GISBrowserGeoportalKanton or map.geo.admin@MapsSwitzerlandSwiss which then was abolished due to the time limitation
