...
The core of the architecture is a Transformer encoder block consisting of _num_layers_ stacked layers of multi-head self-attention and feedforward sub-layers. Here we used the built in Transformer module of the PyTorch Library @PyTorchFoundation.
These layers allow each time step to attend to all others in the sequence, enabling the model to capture both short- and long-term dependencies without recurrence. The encoder is applied in a symmetric fashion (_x_ is passed as both source and target), as no autoregressive decoder is needed for our forecasting task.
